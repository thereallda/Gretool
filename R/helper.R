...
#'
#' @param object 
#' @param col.data \code{data.frame} with at least three columns (indicate condition, enrich and biological groups). 
...
#' @param norm.method Normalization methods to use 
#' @param test.gene.id List of gene ids to be tested 
#' @param assay.colname Name of the assay column in \code{col.data}, e.g., "enrich". 
#' @param biology.colname Name of the biological column in \code{col.data}, e.g., "bio". 
...
#' @param adjust Whether to perform adjustment based on input expression levels.
#' @param p.cutoff P values Cutoff for significant differential modification genes. 
#' @param pseudo.count Minimal offset to prevent negative values. 
#' @param eps Threshold to prevent negative values, default: 1e-4
#' @param shrink_lambda Factor to control the shrinkage of expression size factor from input, default: 0. 
#'
#' @return List of differential modification results
#' @export
#'
DiffModification <- function(object,
                             col.data,
                             contrast.df,
                             norm.method=NULL,
                             test.gene.id=NULL,
  ...
  if (!is.null(norm.method)) {
    data <- Gretool::Counts(object, slot='sample', method=norm.method)
  } else {
    data <- Gretool::Counts(object, slot='sample', method='Raw')
  }
  
  if (!is.null(test.gene.id)) {
    data <- data[rownames(data) %in% test.gene.id,]
  } else {
    data <- data
  }
  
  # split data
  data_enrich <- data[,grep(enrich.id, col.data[,assay.colname])]
  data_input <- data[,grep(input.id, col.data[,assay.colname])]
  
  # adjust enrich data by normalized input data
  if (adjust) {
    # compute gene-wise size factor
    sizeF <- .expressionFactor(data_input, eps=eps)
    sizeF <- .shrinkClip(sizeF, shrink_lambda = shrink_lambda)
    data_enrich <- data_enrich / sizeF
  }
  # design matrix
  bio_group <- col.data[grep(enrich.id, col.data[,assay.colname]), biology.colname]
  design_df <- data.frame(condition=bio_group)
  
  # differential test based on linear model
  deg <- edgeRDE(round(data_enrich),
                 group = bio_group,
                 contrast.df = contrast.df,
                 design.formula = ~0+condition,
                 norm.factors = rep(1, ncol(data_enrich)), # no normalization during testing
                 only.pos = F)
  
  # get enrichment data 
  if (!is.null(object@enrichment)) {
    res.ls <- getEnrichment(object, slot="sample", filter=F)
    names(res.ls) <- gsub('\\..*','',names(res.ls))
    # get enriched genes
    # res.sig.ls <- getEnrichment(object, slot="sample", filter=T)
    # enrich_ids <- unique(unlist(lapply(res.sig.ls, function(x) x$GeneID)))
  } else {
    stop('You should perform `FindEnrichment` before `DiffEnrichment`.')
  }
  
  megred <- lapply(names(deg$res.ls), function(i) {
    df1 <- .mergeDiffModWithEnrich(
      diffmod_res = deg$res.ls[[i]],
      enrich_list = res.ls,
      fc.thresh = log2(1.3),
      contrast_name = i
    )
  ...
  return(megred)
  # return(list(deg, res.ls))
}

#' Size factor based on input expression
#'
#' @param data_input 
#' @param eps see description in \code{DiffModification}
#'
#' @return vector of expression factors
#'
.expressionFactor <- function(data_input, eps = 1e-4) {
  t(apply(data_input, 1, function(x) {
    x2 <- .rmzero(x, eps)
    size <- x2 / mean(x2)
    size
  }))
...
#'
#' @param size_factor from \code{.expressionFactor}.
#' @param shrink_lambda see description in \code{DiffModification}.
#' @param min_factor minimum factor to clip, default: 1e-6.
#' @param max_factor maximum factor to clip, default: 1e6.
#'
#' @return shrunk expression factors
.shrinkClip <- function(size_factor, 
                       shrink_lambda = 0, 
                       min_factor = 1e-6, 
                       max_factor = 1e6) {
  # shrink_lambda in [0,1], 0=no shrink, 1=full shrink to 1
  size_factor2 <- as.matrix(size_factor)
  size_factor2 <- (1 - shrink_lambda) * size_factor2 + shrink_lambda * 1
  size_factor2[size_factor2 < min_factor] <- min_factor
  size_factor2[size_factor2 > max_factor] <- max_factor
  size_factor2
}
...
#'
#' @return vector
.rmzero <- function(x, eps = 1e-4) {
  # replace zero or negative values by eps
  x[is.na(x) | x <= 0] <- eps
  x
}
...
#'
#' @param diffmod_res Table of differential modification results from \code{DiffModification}.
#' @param enrich_list Enriched genes from \code{FindEnrichment}
#' @param contrast_name Contrast name, e.g., "A_B".
#' @param fc.thresh Fold-change threshold for significant differential modification. 
#' @param p.thresh P values threshold for significant differential modification. 
#'
#' @return list 
.mergeDiffModWithEnrich <- function(
    diffmod_res,      
    enrich_list,     
    contrast_name,   
    fc.thresh = log2(1.2),
  ...
  # Extract the two condition names from contrast name
  parts <- unlist(strsplit(contrast_name, "_"))
  cond1 <- parts[1]
  cond2 <- parts[2]
  
  # Pull enrichment DE results for both conditions
  if (any(!parts %in% names(enrich_list))) {
    stop('Differential group names must align with group names in conditions.')
  }
  
  enr1 <- enrich_list[[cond1]][, c("GeneID", "logFC", "FDR")]
  enr2 <- enrich_list[[cond2]][, c("GeneID", "logFC", "FDR")]
  
  # rename for clarity
  colnames(enr1)[2:3] <- paste0(cond1, c(".logFC", ".FDR"))
  colnames(enr2)[2:3] <- paste0(cond2, c(".logFC", ".FDR"))
  
  # Merge everything
  merged <- enr1 %>%
    left_join(enr2, by = "GeneID") %>%
    left_join(diffmod_res, by = "GeneID")
  
  # Compute enrichment FC difference
  merged$logFC.diff <- merged[[paste0(cond1, ".logFC")]] -
    merged[[paste0(cond2, ".logFC")]]
  
  # rename mod FDR if needed
  names(merged)[names(merged) == "FDR"] <- "mod.FDR"
  
  # Classify Up / Down / NS
  merged$mod.type <- "NS"
  merged$mod.type[which(merged$logFC.diff >= fc.thresh & merged$mod.FDR < p.thresh)] <- "Up"
  merged$mod.type[which(merged$logFC.diff <= -fc.thresh & merged$mod.FDR < p.thresh)] <- "Down"
  merged$mod.type <- factor(merged$mod.type, levels=c('Up','Down','NS'))
  merged <- merged[order(merged$mod.type, decreasing = T),]
  return(merged)
}
...
#' 
#' @name FindEnrichment 
#' @aliases FindEnrichment  FindEnrichment,Gretool,character,character,numeric,numeric-method
#' 
...
FindEnrichment <- function(object, slot=c("sample","spike_in"), norm.method, 
  ...
  if (is.null(names(object@norm_factors[[slot]]))) {
    stop("Normalizations for ", slot, " not found. At least one normalization should be performed.")
  }
  norm.method <- match.arg(norm.method, choices = names(object@norm_factors[[slot]]))
...
UseNormalization <- function(object, slot=c("sample","spike_in"), method) {
  ...
  object@norm_factors[[slot]][[method]] <- list(normFactor=counts_scale$normFactor,
...
#' @param prior.count Average prior count to be added to observation to shrink the estimated log-fold-changes towards zero, default: 0.125.
...
edgeRDE <- function(counts, 
  ...
  de.ls1 <- pbapply::pblapply(1, function(i) {
    ...
    fit.glm <- edgeR::glmFit(degs, design = design.mat, prior.count = prior.count)
...
#'
#' @param x Input data, can be counts matrix or object.
...
#'
#' @return Filtered count matrix or updated object.
#' @export
#'
FilterLowExprGene <- function(x, group=NULL, min.count=10) {
  ...
  if (is.matrix(x) | is.data.frame(x)) { 
    data <- as.matrix(x)
  } else if ("Gretool" %in% as.character(class(x))) {
  ...
  if ("Gretool" %in% as.character(class(x))) {
    x <- .keepConsRows(x)
  }
...
.keepConsRows <- function(object) {
  ...
  data_nsp <- data[grep(paste(c(spike.in.prefix, synthetic.id), collapse = "|"), rownames(data), invert = TRUE),] # data from samples
  data_sp <- data[.safe_grep(spike.in.prefix, rownames(data)),] # data from spike-in
  ...
  if (!all(rownames(data_nsp) %in% rownames(data))) {
    data_nsp <- data_nsp[intersect(rownames(data_nsp), rownames(data)), ]
  }
  
  object@counts$sample[["Raw"]] <- data_nsp
...
#' @param score Vector of performance scores from normalization score. 
...
BetweenStatPlot <- function(data, x, y, color, palette = NULL,
  ...
  if (exists("stat_dat")) {
    p <- p + ggpubr::stat_pvalue_manual(data = stat_dat, label = add.p, tip.length = 0, size = 3)
  }
...
#' Safe grep
#'
#' @param pattern Same as \code{grep}. character string containing a regular expression (or character string for fixed = TRUE) to be matched in the given character vector. 
#' @param x Same as \code{grep}. a character vector where matches are sought, or an object which can be coerced by as.character to a character vector. 
#' @param ... Other parameters passed to \code{grep}
#'
#' @return matched value
#'
.safe_grep <- function(pattern, x, ...) {
  if (isTRUE(nzchar(pattern))) grep(pattern, x, ...) else NULL
}
